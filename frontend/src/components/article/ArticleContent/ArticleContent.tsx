...
const ArticleContent = ({ article, category, articleId }: ArticleContentProps) => {
	const { handleClickFillHeart, handleClickEmptyHeart, isLike, likeCount } = useHeartClick({
		prevIsLike: article?.isLike || false,
		prevLikeCount: article?.likeCount || 0,
		articleId,
	});
				...
				<S.Container>
					<S.Header>
						<S.CategoryTitle
							tabIndex={0}
							aria-label={`${category}`}
							isQuestion={category === '질문'}
						>
							{category}
						</S.CategoryTitle>
						<S.UserProfile tabIndex={0}>
							<S.UserProfileImg src={article.author.avatarUrl} alt="작성자의 프로필 이미지입니다" />
