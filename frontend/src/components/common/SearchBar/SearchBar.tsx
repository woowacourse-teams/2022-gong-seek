...
const SearchBar = ({ isValid} : { isValid: boolean}) => {
	...
	const onSubmitSearchTarget = () => {
		setSearchInputState({isSearching: true, target: searchInput, isSearchOpen: searchInputState.isSearchOpen});
	}

	return (
		<S.Container>
			<S.SearchInput type="text" readOnly={isValid} value={searchInput} onChange={(e) => {onChangeInputValue(e)}}/>
